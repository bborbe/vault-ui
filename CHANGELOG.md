...
- feat(ui): Task cards show how long it has been since anything last happened on them — a small grey duration (`<1m`, `47m`, `2h`, `4d`, `3w`) in the card footer, with the full timestamp on hover. The timestamp is the newer of the task file's mtime and the mtime of the Claude session transcript named by the task's `claude_session_id` (new `activity.py`, surfaced as `activity_date` on `TaskResponse`). Neither signal works alone: the file only changes when something is written to it, so an agent mid-turn looks stale for hours, while the transcript keeps a dead session's timestamp once that session ends. Tasks with no `claude_session_id` (human tasks) and sessions whose transcript is not on this machine (cloud/container) fall back to the file mtime. Bumps the `app.js` and `style.css` cache-bust tokens.
