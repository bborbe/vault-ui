...
- **take-over of a Starting card** — `POST /api/tasks/{id}/take-over` and
  ...
  **That write must be the last one to the file.** The take-over SIGTERMs the
  launch, and `vault-cli work-on` answers a failed turn with a compensating
  clear — it re-reads the task and deletes `claude_session_id` plus that run's
  metrics entry ("a failed turn must not leave a resumable-looking id on disk",
  `pkg/ops/workon.go` → `clearSessionAndMetrics`). Two writers with opposite
  intents, and the launcher's can land after ours. When it does, the card falls
